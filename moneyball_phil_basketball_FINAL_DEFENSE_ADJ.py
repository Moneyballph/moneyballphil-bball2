...
# MoneyBall Phil Basketball Simulator (FINAL BUILD w/ Recent Avg + Defense Tier Adjustment)
...
def defense_tier_multiplier(rank):
    if 1 <= rank <= 10:
        return 0.97  # Tough defense, reduce 3%
    elif 11 <= rank <= 20:
        return 1.00  # Neutral
    else:
        return 1.03  # Easy defense, boost 3%

# Simulation Logic
if st.button("Simulate"):
    base = (points + rebounds + assists) / 3 if stat_type == "PRA" else points
    combined_avg = (base + recent_avg) / 2
    defense_multiplier = defense_tier_multiplier(defense_vs_position)
    raw_hit_chance = combined_avg / sportsbook_line
    hit_chance = round(min(0.99, max(0.01, raw_hit_chance * defense_multiplier)), 4)
