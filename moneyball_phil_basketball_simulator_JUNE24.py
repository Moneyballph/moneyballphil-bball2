...
# MoneyBall Phil Basketball Simulator (JUNE 24 FINAL BUILD)
...
if st.button("Simulate"):
    ...
    result = {
        ...
        "Over %": f"{implied_over * 100:.1f}%" if implied_over else "-",
        "Under %": f"{(1 - implied_under) * 100:.1f}%" if implied_under else "-",
        "Line": sportsbook_line,
        "Recent Avg": recent_avg,
        "Over EV": f"{ev_over:.1f}%" if ev_over else "-",
        "Under EV": f"{ev_under:.1f}%" if ev_under else "-",
        "Zone": get_hit_zone(hit_chance * 100)
    }
