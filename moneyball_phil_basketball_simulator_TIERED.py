# MoneyBall Phil Basketball Simulator (FINAL BUILD w/ Defense Tier Fix)
...
if st.button("Simulate"):
    ...
    tier_label, tier_adjust = get_defense_tier(defense_vs_position)
    raw_prob = combined_avg / sportsbook_line if sportsbook_line > 0 else 0
    hit_chance = round(min(0.99, max(0.01, raw_prob + tier_adjust)), 4)
    ...
    st.success(f"{player_name} - True Hit Probability: {hit_chance * 100:.2f}% ({tier_label})")
