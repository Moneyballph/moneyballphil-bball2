...
# MoneyBall Phil Basketball Simulator (FINAL BUILD w/ Recent Avg + Defense Tier Adjustments)
...
def defense_tier_multiplier(rank):
    if rank >= 21:  # Easier defense
        return 1.05
    elif rank >= 11:  # Average defense
        return 1.00
    else:  # Tough defense
        return 0.94
...
if st.button("Simulate"):
    ...
    multiplier = defense_tier_multiplier(defense_vs_position)
    adjusted_avg = combined_avg * multiplier
    hit_chance = round(min(0.99, max(0.01, adjusted_avg / sportsbook_line)), 4)
